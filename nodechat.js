...
function login(username,password,callback) {
	User.findOne({username:username,password:password},function(err,doc){
		console.log("look up username:"+username);
		if (doc) {
...
function login_session(client,user,callback) {
	UserSession.findOne({session:client.sessionId, valid:true},function(err,doc){
		console.log("login session: "+user.username)
		var session = false;
		if (doc) {
			session = doc;
		}
		...
		session.save(function(err){
			console.log("login client session: "+client.sessionId +" err:"+JSON.stringify(err));
			callback(true);
		});
	});
...
function get_user_session(client,callback) {
	UserSession.findOne({session:client.sessionId, valid:true},function(err,doc){
		console.log("get client session: "+client.sessionId+" doc:"+JSON.stringify(doc));
		if (doc) 
			callback(doc);
		else
			callback(false);
...
function get_active_users(clients,callback,final_callback) {
	if(clients) {
		...
		console.log("get active users from clients");
		UserSession.findOne({session:keys[0], valid:true},function(err,doc){
			console.log("get active users: docs:"+JSON.stringify(doc));
			if (doc)
				callback(doc);
			else
				callback(false);
		...
		console.log("get active users from no client");
...
function get_last_messages(count,callback){
	console.log("getting last "+count+" messages");
	
	Conversation.find().sort('date',-1).limit(count).find(function (err,messages) {
		console.log("last "+count+" messages: "+ JSON.stringify(messages));
		if(err)console.log("error last "+count+" messages: "+ JSON.stringify(err));
		if(messages)
			callback(messages);
		else
			callback(false);
